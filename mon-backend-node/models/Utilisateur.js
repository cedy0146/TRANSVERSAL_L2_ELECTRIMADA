...
class Utilisateur {
    ...
    /**
     * Creer un nouvel utilisateur (avec hash du PIN)
     */
    static async create(data) {
        const { nom, role, pin, id_foyer } = data;
        const pin_hash = hashPin(pin); // Hacher le PIN avec SHA256
        const foyerValue = id_foyer || null;
        
        const [result] = await pool.query(
            'INSERT INTO Utilisateur (nom, role, pin, id_foyer) VALUES (?, ?, ?, ?)',
            [nom, role || 'chef_foyer', pin_hash, foyerValue]
        );
    ...
    /**
     * Verifier le PIN d'un utilisateur
     */
    static async verifyPin(nom, pin) {
        const utilisateur = await Utilisateur.getByNom(nom);
        if (!utilisateur) {
            return null;
        }
        // Verifier le PIN avec la fonction comparePin
        if (comparePin(pin, utilisateur.pin)) {
            return utilisateur;
        }
        return null;
    }
